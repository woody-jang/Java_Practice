...
둘째 줄부터 N개의 줄에는 파일 이름이 주어진다. N은 50보다 작거나 같은 자연수이고
파일 이름의 길이는 모두 같고 길이는 최대 50이다. 파일이름은 알파벳과 "." 그리고 "?"로만 이루어져 있다.
...
public class _1032_CommandPrompt {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < inputWords.get(0).length(); i++) {
			char last = inputWords.get(0).charAt(i);
			for (int j = 1; j < inputWords.size(); j++) {
				if (last != inputWords.get(j).charAt(i)) {
					sb.append("?");
					break;
				}
			}
			if (sb.toString().length() != i + 1) {
				sb.append(last);
			}
		}
