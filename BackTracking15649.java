...
// for문을 두번째로 입력한 숫자만큼 실행해야되서 고민함
// 재귀밖에 생각이 안나서 재귀를 써봄
// 백준의 재귀 문제들을 풀고난후 재귀를 쓰는데 많이 도움이됨
// 숫자들끼리 겹치면 안되기때문에 list에 재귀 이전의 숫자들을 담음
// list를 출력할때도 활용함
...
public class BackTracking15649 {
	...
	private static void getNumbers(int n, int m, int cnt, List<Integer> postNumbers, StringBuffer sb) {
		if (m > 0) { // 반복횟수가 0보다 크면 반복
			for (int i = 1; i <= n; i++) {
				if (postNumbers.contains(i)) // 이전 재귀에서 추가한 숫자와 겹치면 continue
					continue;
				postNumbers.add(i); // 없다면 list에 현재 숫자 추가
